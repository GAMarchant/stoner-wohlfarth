...
#USAGE: ***************************************
#./run-cubic-SW.sh results_directory results_prefix H_diff H_n H_theta H_phi K_theta K_phi
...
#Factor to convert between degrees and radians
deg2rad=$(bc -l <<< "3.14159265359/180")

#Set field direction (polar)
H_theta=$5; H_phi=$6
H_x=$(bc -l <<< "s($H_theta*$deg2rad)*c($H_phi*$deg2rad)"); 
H_y=$(bc -l <<< "s($H_theta*$deg2rad)*s($H_phi*$deg2rad)"); 
H_z=$(bc -l <<< "c($H_theta*$deg2rad)")

#Set field direction (polar)
K_theta=$7; K_phi=$8
K_x=$(bc -l <<< "s($K_theta*$deg2rad)*c($K_phi*$deg2rad)"); 
K_y=$(bc -l <<< "s($K_theta*$deg2rad)*s($K_phi*$deg2rad)"); 
K_z=$(bc -l <<< "c($K_theta*$deg2rad)")
...
temp_file="cubic_SW_polar.template"
...
do
  ...
  file_pref=$results_dir/$pref.H-$H_theta-$H_phi.K-$H_theta-$H_phi.T-$temp
  ...
  sed -i "s/HFIELDTHETA/$H_theta/g" $file_pref.in
  sed -i "s/HFIELDPHI/$H_phi/g" $file_pref.in
  sed -i "s/HFIELDX/$H_x/g" $file_pref.in
  sed -i "s/HFIELDY/$H_y/g" $file_pref.in
  sed -i "s/HFIELDZ/$H_z/g" $file_pref.in

  sed -i "s/KCTHETA/$K_theta/g" $file_pref.in
  sed -i "s/KCPHI/$K_phi/g" $file_pref.in
  ...
  echo "#1:H(T) 2:Htheta(deg) 3:Hphi(deg) 4:Hx 5:Hy 6:Hz 7:Kc1(eV/atom) 8:Kc2(eV/atom) 9:Kctheta(deg) 10:Kcphi(deg) 11:Kcx 12:Kcy 13:Kcz 14:magnorm 15:magtheta(deg) 16:magphi(deg) 17:magx 18:magy 19:magz 20:emag(eV)" > $file_pref.dat
